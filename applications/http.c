 ...
 */
void wlan_ready_handler_http(int event, struct rt_wlan_buff *buff, void *parameter)
{
 ...
 */
void wlan_station_disconnect_handler_http(int event, struct rt_wlan_buff *buff, void *parameter)
{
    LOG_I("disconnect from the network!");
}


static void http_test(void *parameter)
{
    int result = RT_EOK;
    ...
    rt_wlan_register_event_handler(RT_WLAN_EVT_READY, wlan_ready_handler_http, RT_NULL);
    /* 注册 wlan 网络断开连接的回调 */
    rt_wlan_register_event_handler(RT_WLAN_EVT_STA_DISCONNECTED, wlan_station_disconnect_handler_http, RT_NULL);

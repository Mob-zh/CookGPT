...
static rt_err_t drv_pwm_set(TIM_HandleTypeDef *htim, struct rt_pwm_configuration *configuration)
{
    ...
    rt_kprintf("tim_clock=%d period=%d\n",tim_clock,period);

    psc = period / MAX_PERIOD + 1;
    period = period / psc;
    rt_kprintf("psc=%d period=%d\n",psc,period);
    ...
    rt_kprintf("hwPSC=%d hwARR=%d\n",htim->Instance->PSC,htim->Instance->ARR);
...
//     stm32_pwm_obj[PWM14_INDEX].channel |= 1 << 1;
// #endif
